  ...
  {{-- Start table --}}
    ...
    <div class="d-flex bd-highlight mb-3">
      ...
      <div class="bd-highlight ml-auto">
          <a href="{{ route('brand.create') }}" role="button" class="btn btn-success">
              <unicon name="book-medical" fill="#F7F7F7" width="20" height="20" ></unicon>  إضافة ماركة جديدة
          </a>
      </div>
    </div>
    ...
    <table class="table table-bordered table-hover border-0 dtbl-ahmad text-center table-control">
      <thead>
        <tr class="bg-dark text-light border-0">
          ...
          <th scope="col">أسم الماركة <unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=brands.name'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
          <th scope="col">عدد المنتجات<unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=brands.number_products'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
          <th scope="col" width="90">الحالة <unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=brands.is_active'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
              <unicon onclick="window.location='{{ route('brand.edit',$val->id)}}'" class="icon_vue_mein" name="edit" fill="#808080" width="22" height="22" ></unicon>
              <unicon onclick="destroy_item('{{ route('brand.destroy',$val->id)}}')" class="icon_vue_mein" name="trash-alt" fill="#808080" width="22" height="22" ></unicon>
          </td>
        </tr>
