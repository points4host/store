  ...
  {{-- Start table --}}
    ...
    <div class="d-flex bd-highlight mb-3">
      ...
      <div class="bd-highlight ml-auto">
          <a href="{{ route('category.create') }}" role="button" class="btn btn-success">
              <unicon name="book-medical" fill="#F7F7F7" width="20" height="20" ></unicon>  إضافة تصنيف جديد
          </a>
      </div>
    </div>
    ...
    <table class="table table-bordered table-hover border-0 dtbl-ahmad text-center table-control">
      <thead>
        <tr class="bg-dark text-light border-0">
          ...
          <th scope="col">أسم القسم <unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=categories.name'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
          <th scope="col">رئيسي/فرعي <unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=categories.sub'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
          <th scope="col">عدد المنتجات<unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=categories.sub'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
          <th scope="col" width="90">الحالة <unicon onclick="window.location='{{$db->path()}}?orderby={{$html_orderby}}&order_value=categories.is_active'" class="{{$css_orderby}}" name="sort-amount-up" fill="#b8c7ce" ></unicon></th>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{ $val->name }}</td>
          <td>@php echo check_main($val->sub) ; @endphp</td>
          <td>0</td>
          <td>@php echo check_status($val->is_active) ; @endphp</td>
          <td>
              <unicon onclick="window.location='{{ route('category.edit',$val->id)}}'" class="icon_vue_mein" name="edit" fill="#808080" width="22" height="22" ></unicon>
              <unicon onclick="destroy_item('{{ route('category.destroy',$val->id)}}')" class="icon_vue_mein" name="trash-alt" fill="#808080" width="22" height="22" ></unicon>
          </td>
        </tr>
